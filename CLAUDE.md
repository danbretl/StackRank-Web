...
- **Cache-busting:** when you change JS or CSS, **bump `app.js?v=N` / `styles.css?v=N` in `index.html`** — otherwise GitHub Pages and browsers serve stale assets. Current: `app.js?v=111`, `styles.css?v=74`. **Note:** `app.js` imports some `lib/` modules with their own `?v=N` (e.g. `lib/backup.js?v=1`, `lib/packs.js?v=2`) — bump that query too when you change one of those modules, since bumping only `app.js?v=` won't refresh a cached `lib/` import.
