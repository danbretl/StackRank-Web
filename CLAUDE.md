...
- **`lib/`** — DOM-free ES modules extracted from `app.js` and shared by the browser app plus tests (`zip`, `text`, `format`, `movie`, `persistence`, `insights`, `packs`, `share-export`, `share-svg`, `ranking`, `undo`, `backup`).
...
- **Ranking:** binary-insertion comparison; **Undo last choice** / **Cancel ranking** (only one shows at a time; cancel restores the movie + the pre-ranking scroll position). Custom **pointer-based drag** reorder (handle-only on touch, drag-anywhere on desktop). The **full-screen ranking** is an interactive responsive poster grid with detail access, re-rank/remove, title/year filtering, Comfortable/Compact density, jump-to-rank, arrow/Home/End navigation, and undoable 2-D pointer drag (disabled while filtered).
