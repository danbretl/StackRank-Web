...
export function normalizeDogProfile(value) {
  ...
  const shortDescription = cleanText(value.shortDescription);
  ...
  if (
    summary.length < 20 ||
    summary.length > 700 ||
    (interestingFact.length > 0 && interestingFact.length < 10) ||
  ...
  return {
    summary,
    shortDescription: shortDescription.length <= 180 ? shortDescription : "",
    interestingFact,
    sizeBand,
...
export function dogProfileSourceLinks(profile, sources) {
  const ids = new Set(Array.isArray(profile?.sourceIds) ? profile.sourceIds : []);
  return (Array.isArray(sources) ? sources : []).flatMap((source) => {
    if (source?.kind !== "breed-reference" || !ids.has(source.id) || !cleanText(source.name)) return [];
    try {
      const url = new URL(source.url);
      if (url.protocol !== "https:" || url.username || url.password) return [];
      return [{ title: cleanText(source.name), url: url.href }];
    } catch { return []; }
  });
}
