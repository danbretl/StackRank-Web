...
export const RANKING_VIEW_MODES = Object.freeze(["detailed", "posters", "compact"]);

export function normalizeRankingViewMode(value, fallback = "detailed") {
  if (RANKING_VIEW_MODES.includes(value)) return value;
  return RANKING_VIEW_MODES.includes(fallback) ? fallback : "detailed";
}

export function rankingViewLabel(value) {
  const normalized = normalizeRankingViewMode(value);
  if (normalized === "posters") return "Posters";
