...
const RANKING_MILESTONES = Object.freeze([1, 2, 5, 10, 25, 50, 75, 100, 150, 200, 250, 500, 1000]);

export function rankingMilestoneProgress(count) {
  const normalized = Math.max(0, Math.floor(Number(count) || 0));
  const next = RANKING_MILESTONES.find((milestone) => milestone > normalized) ||
    Math.ceil((normalized + 1) / 500) * 500;
  const previous = [...RANKING_MILESTONES].reverse().find((milestone) => milestone <= normalized) || 0;
  const span = Math.max(1, next - previous);
  return {
    count: normalized,
    previous,
    next,
    remaining: Math.max(0, next - normalized),
    progress: Math.max(0, Math.min(1, (normalized - previous) / span)),
  };
}

export function countRankedInMonth(movies, now = new Date()) {
  const date = now instanceof Date ? now : new Date(now);
  if (Number.isNaN(date.getTime())) return 0;
  const year = date.getFullYear();
  const month = date.getMonth();
  return (Array.isArray(movies) ? movies : []).filter((movie) => {
    if (!movie?.rankedAt) return false;
    const ranked = new Date(movie.rankedAt);
    return !Number.isNaN(ranked.getTime()) && ranked.getFullYear() === year && ranked.getMonth() === month;
  }).length;
}
