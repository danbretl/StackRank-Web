...
export function reconcilePackProgressCompletion(
  entry = {},
  { completed = false, packVersion = null, completedAt = null } = {},
) {
  const normalized = normalizePackProgressEntry(entry);
  if (completed && !normalized.completedAt) {
    return {
      changed: true,
      entry: {
        ...normalized,
        completedAt,
        packVersionSeen: packVersion,
      },
    };
  }
  if (!completed && normalized.completedAt) {
  ...
  return { changed: false, entry: normalized };
}
