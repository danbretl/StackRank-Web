...
const shuffledWith = (values, random) => {
...
// A returning customer's homepage should feel fresh on every visit without
// hiding useful continuation work. Keep up to two explicitly engaged packs,
// keep at most one organic head start, then favor category variety while
// filling the remaining slots from the entire unfinished library. The injected
// random function makes this behavior deterministic in tests.
export function selectHomepagePackEntries(packEntries, { limit = 6, random = Math.random } = {}) {
  const count = Math.max(0, Math.floor(Number(limit) || 0));
  if (!count) return [];

  const unfinished = shuffledWith(
    (Array.isArray(packEntries) ? packEntries : []).filter(({ pack, stats }) => pack?.slug && stats && !stats.completed),
    random,
  );
  if (unfinished.length <= count) return unfinished;

  const selected = [];
  const selectedSlugs = new Set();
  const take = (entry) => {
    if (!entry || selectedSlugs.has(entry.pack.slug) || selected.length >= count) return false;
    selected.push(entry);
    selectedSlugs.add(entry.pack.slug);
    return true;
  };

  const engaged = unfinished.filter(({ stats }) => stats.resurfaced || stats.started);
  const headStarts = unfinished.filter(({ stats }) => stats.discovered || (!stats.started && stats.handled > 0));
  const engagedLimit = Math.min(2, Math.ceil(count / 3));
  engaged.slice(0, engagedLimit).forEach(take);
  take(headStarts.find(({ pack }) => !selectedSlugs.has(pack.slug)));

  const selectedCategories = new Set(selected.map(({ pack }) => pack.category).filter(Boolean));
  unfinished.forEach((entry) => {
    if (selected.length >= count) return;
    const category = entry.pack.category || "";
    if (!category || selectedCategories.has(category)) return;
    if (take(entry)) selectedCategories.add(category);
  });
  unfinished.forEach(take);

  return shuffledWith(selected, random);
}
