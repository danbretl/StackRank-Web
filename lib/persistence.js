...
export const REMOTE_LIST_PAYLOAD_LIMIT_BYTES = 1024 * 1024;
export const REMOTE_PACK_PROGRESS_STATE_LIMIT_BYTES = 8 * 1024;
...
export function isJsonPayloadWithinByteLimit(value, limitBytes) {
  return jsonByteLength(value) <= limitBytes;
}
...
const normalizedRankingPayloads = (payloads = []) =>
  newestFirst(
...
export function mergeRankingPayloadsWithMetadata(payloads = []) {
  const sorted = normalizedRankingPayloads(payloads);
  const [newest = emptyRankingPayload(), ...older] = sorted;
  const appendedMovies = [];
  const seen = new Set(newest.movies.map(movieKey));
  const movies = older.reduce((mergedMovies, payload) => {
    payload.movies.forEach((movie) => {
      const key = movieKey(movie);
      if (!seen.has(key)) {
        seen.add(key);
        appendedMovies.push(movie);
      }
    });
    return mergeRankings(mergedMovies, payload.movies);
  }, newest.movies);
  ...
  return {
    ...
    appendedMovies,
    appendedKeys: appendedMovies.map(movieKey),
  };
}

export function mergeRankingPayloads(payloads = []) {
  const { movies, updated_at } = mergeRankingPayloadsWithMetadata(payloads);
  return { movies, updated_at };
}
