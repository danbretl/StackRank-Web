...
| 2026-07-10 | Separate pack dismissal from pack-to-pack navigation | A back arrow used for returning to All Packs was visually grouped with previous/next arrows and could not communicate whether it dismissed the sheet or changed packs | Pack detail now places an explicit `× Close` control in the upper-left and keeps optional Prev / Pack x of y / Next navigation in its own pager |
...
| 2026-07-10 | Pack dismissal and CTA hierarchy follow-up | Replaced the ambiguous pack back arrow with an upper-left `× Close`, separated the filtered pack pager, and replaced harsh black routine CTA fills with the editorial double-keyline primary hierarchy across Add/Search, packs, Rank actions, detail, Share, sign-in, import, Tonight, full-screen jump, and public shared lists; bumped `app.js?v=179`, `styles.css?v=145` | In-app Browser verified Rank, Discover, and pack detail; deterministic desktop/iPad/iPhone screenshots covered main, All Packs, pack detail, and Share; focused primary-action computed-style regression added; full `npm run verify` passed (229 Node, 21 Deno, 28 E2E) | Run production smoke after deployment and monitor physical devices for perceived primary/secondary contrast |
