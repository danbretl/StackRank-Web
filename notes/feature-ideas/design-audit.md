...
| 2026-07-10 | Separate pack dismissal from pack-to-pack navigation while matching All Packs | A back arrow used for returning to All Packs was visually grouped with previous/next arrows and could not communicate whether it dismissed the sheet or changed packs; the first `× Close` treatment then introduced a second dismissal pattern | Pack detail now reuses the All Packs icon-only `×` in the upper-right and fits optional Prev / Pack x of y / Next navigation into the toolbar to its left |
