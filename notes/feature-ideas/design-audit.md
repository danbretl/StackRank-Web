...
- **Implementation status:** Production release/post-release review complete; cross-device responsive/modality audit implemented
...
| Ranking review Swap icon rendered as an unclear glyph | Supplied desktop screenshot showed the Swap button icon reading like a broken mark instead of a recognizable action | Removed the ambiguous custom `icon-swap` artwork and made the review Swap command text-only, matching the adjacent Keep order and End review controls | Focused ranking-review E2E now asserts the Swap control has no SVG/icon child; in-app Browser local review screenshot archived at `debug/screenshots/runs/2026-07-01T0816Z-review-swap-icon`; production deployment `dpl_3N3uKvApJEyi8FPQTxTH8k9M28ov` reached READY and `npm run test:production` passed |
| Full-screen ranking actions still used a hover-only/poster-overlay model | Supplied desktop screenshot showed Re-rank/Remove persisting after drag, while the reorder control sat over poster art and the action model diverged from the main ranking list | Replaced the fullscreen card hover affordance with always-visible segmented `Info / Re-rank / Move / Remove` text controls, moved drag to the `Move` segment, disabled Move while filtered, and removed the poster-overlay reorder button | Focused fullscreen E2E asserts visible fitted labels, no drag handle inside posters, filtered action state, direct Info detail opening, and no stale drag state after reorder; final local screenshot `reports/e2e/runs/2026-07-01T083808Z/screenshots/fullscreen-ranking-interactions.png`; `npm run verify` passed; cache bumped to `app.js?v=149`, `styles.css?v=113` |
| Full-screen Jump to focused cards used Chrome-blue focus color | Supplied desktop screenshot showed rank #51 focused with a bright blue outline that broke the monochrome design system | Changed the shared focus token to black and moved fullscreen jump focus to a card-level monochrome ring so the focused movie cell reads as a StackRank state, not a browser default | Focused fullscreen E2E submits Jump to, asserts the focused card shadow is `rgb(17, 17, 17)` and not blue, and archives `reports/e2e/runs/2026-07-01T085058Z/screenshots/fullscreen-ranking-jump-focus.png`; `npm run verify` passed; cache bumped to `styles.css?v=114` |
| Full-screen drag reorder Undo toast clicked through to the movie grid | User report after accidental fullscreen drag: clicking `Undo` hit the UI underneath the toast instead of undoing the reorder | Moved the global feedback mount out of the app shell, made action toasts explicitly hit-testable, and kept passive status toasts click-through | Focused fullscreen E2E now drags a card, verifies `elementFromPoint` at the Undo button resolves to Undo, clicks it by mouse coordinates, and asserts the original order is restored; full `npm run verify` passed; screenshot archived at `reports/e2e/runs/2026-07-01T090512Z/screenshots/fullscreen-ranking-interactions.png`; cache bumped to `app.js?v=150`, `styles.css?v=115` |
| Full-screen ranking still competed with mobile app chrome | Supplied mobile fullscreen screenshot showed the bottom tab bar visible over the supposedly fullscreen workspace, reducing usable poster grid height and making the fullscreen surface feel less dense than the main list | Added an explicit `is-fullscreen-open` body state, hid the mobile tab bar for that mode, raised all modal overlays above app chrome, and tightened the mobile fullscreen header/control heights | Focused fullscreen E2E now asserts the fullscreen body state, hidden mobile nav, overlay z-order above the nav, compact-vs-comfortable density, and a bounded mobile header height; full `npm run verify` passed; local rendered smoke loaded `app.js?v=154` and `styles.css?v=119` with no console warnings/errors |
| Mobile ranking overflow menus could render underneath later row controls | Supplied mobile screenshots showed the open first/second-row overflow menu tucked under the following rows' ellipsis buttons because each ranked row creates its own stacking context | Added a shared `is-overflow-open` layer state for movie overflow containers and raised active ranking, queue, and fullscreen card shells above sibling rows while open | App-shell E2E now saves a mobile open-menu screenshot and samples the menu's right edge with `elementFromPoint` to catch row-control occlusion; full `npm run verify` passed; cache bumped to `app.js?v=155`, `styles.css?v=120` |
| Full-screen mobile reorder controls rendered as always-on black squares | Supplied mobile full-screen screenshots showed the per-card drag button visible in normal and compact modes, visually reading as broken black poster overlays and diverging from the explicit move-mode model used by the main ranking list | Added a full-screen Move toggle, hid card drag handles until move mode is active, hid overflow actions while moving, and required handle-only drag in full-screen move mode | Focused fullscreen E2E now asserts normal/move/compact/filter states, handle visibility, overflow hiding, handle-only drag, and undo restoration; in-app Browser mobile smoke imported six movies and verified normal, move, and compact full-screen states with no console warnings/errors; full `npm run verify` passed; cache bumped to `app.js?v=156`, `styles.css?v=121` |
| Overflow menus wrapped short action labels and full-screen cards showed an accidental black focus outline | Supplied mobile ranking and full-screen screenshots showed `Re-rank` wrapping in the overflow menu and the full-screen card drawing a black border when its overflow menu was open | Added a higher-specificity shared overflow-action rule so menu actions ignore row icon-button sizing, widened menu minimums to 152px, forced single-line labels, and removed full-card `:focus-within` outlines in favor of a subtle control-level `:focus-visible` state | App-shell E2E now measures mobile overflow action widths/white-space, full-screen E2E asserts no card border/shadow while the menu is open, in-app Browser mobile smoke verified both screenshots' states with no console warnings/errors, and full `npm run verify` passed; cache bumped to `styles.css?v=122` |
| Outside taps on another movie could both dismiss an overflow menu and open detail | User report: when a ranking overflow menu was open, tapping another movie row/card should dismiss the menu only, not trigger that movie's detail action | Moved outside-menu dismissal into a capture-phase boundary that consumes only clicks made while a menu is open, and closed menus immediately when an overflow action is chosen so hidden menus do not absorb the next control click | App-shell and full-screen E2E now use coordinate taps to assert outside-menu movie taps close the menu without opening detail; in-app Browser mobile smoke verified main and full-screen ranking dismissal with no console warnings/errors; full `npm run verify` passed; cache bumped to `app.js?v=157` |
| Movie detail opened from overflow Info could render under the source row/menu | Supplied mobile screenshot showed the detail sheet beneath the original ranked movie row and overflow menu after choosing Info from that row's overflow | Movie detail now closes any active overflow before opening, returns focus to the visible overflow toggle, and sits above raised overflow rows/menus and fullscreen drag ghosts while the lightbox remains topmost | App-shell E2E now asserts ranking and queue overflow Info open detail with no raised row/menu above it; full-screen E2E asserts the same for card overflow Info; in-app Browser mobile smoke verified main and full-screen ranking detail layering with no console warnings/errors; full `npm run verify` passed; cache bumped to `app.js?v=158`, `styles.css?v=123` |
| Ranking overflow Info could leave the menu visible over the detail sheet | Follow-up mobile screenshot showed the ranking overflow menu still displayed above the American Beauty detail sheet after choosing Info | Overflow menu actions now close in the capture phase before their action runs, every close clears stale raised row/card classes, and detail-open modal state hides any overflow menu as a CSS backstop | App-shell E2E now coordinate-taps the ranking overflow Info action and asserts no open or visible menus, no raised rows, and detail hit-testing above the former menu point; in-app Browser mobile smoke verified `app.js?v=159` / `styles.css?v=124`; full `npm run verify` and production smoke passed |
...
- **Latest post-release fullscreen-action deployment:** `dpl_G6zririCHP8SKiy95UsHGLdiexUi`, production URL `https://stackrank-8qntrpmcb-danbretl-2590s-projects.vercel.app`, GitHub commit `98ab41b`. Production smoke passed against `app.js?v=149` and `styles.css?v=113`; live production fullscreen-action screenshot is archived at `debug/screenshots/runs/2026-07-01T0839Z-fullscreen-actions-production/fullscreen-actions-desktop.png`; Vercel reported no runtime error clusters and no warning/error/fatal logs in the deployment window.
- **Latest post-release fullscreen-jump-focus deployment:** `dpl_FqL13b4iC1EdMtBKhHZyRUkJPuNk`, production URL `https://stackrank-38yelekgf-danbretl-2590s-projects.vercel.app`, GitHub commit `03af6f3`. Production smoke passed against `app.js?v=149` and `styles.css?v=114`; live production Jump focus screenshot is archived at `debug/screenshots/runs/2026-07-01T0858Z-jump-focus-production/fullscreen-jump-focus-desktop.png`; Vercel reported no runtime error clusters and no warning/error/fatal logs in the deployment window.
- **Post-release fullscreen-undo deployment:** `dpl_8TwgMkzZzX2HfyFfPp1pcXz2B6yA`, production URL `https://stackrank-mxgdmqqby-danbretl-2590s-projects.vercel.app`, GitHub commit `d5e3789` with app fix commit `7991df1` immediately before it. Production smoke passed against `app.js?v=150` and `styles.css?v=115`; local focused/full E2E screenshot is archived at `reports/e2e/runs/2026-07-01T090512Z/screenshots/fullscreen-ranking-interactions.png`; live production fullscreen seeded smoke is archived at `debug/screenshots/runs/2026-07-01T0910Z-undo-toast-production/fullscreen-seeded-production.png`; Vercel reported no runtime error clusters and no warning/error/fatal logs in the deployment window.
...
- **Runtime/log checks:** Vercel runtime logs for the latest checked production deployments returned no warning, error, or fatal entries in the review windows. Chrome DevTools showed the expected live `app.js?v=150` and `styles.css?v=115` requests, no console errors/warnings beyond the expected build info line, and the seeded fullscreen ranking rendered with the root-level global toast layer available above fullscreen modal content.
...
| 2026-07-02 | Compact mobile ranking and queue rows by promoting common actions and hiding rare actions | Real-device feedback showed an iPhone Pro Max only surfaced about 1.5 ranked movies because headers and per-row controls consumed the first viewport | Mobile Rank keeps Add/Move in the sticky list header, opens detail from row taps, exposes handles only in Move mode, and puts Info/Re-rank/Remove in overflow; mobile Lists keep Rank visible and move Info plus save/hide/remove into overflow while desktop direct controls remain |
| 2026-07-02 | Treat full-screen ranking as a dense browsing grid on mobile | Follow-up screenshots showed the full-screen view still spent more space on per-card controls than on ranked movies, and Comfortable/Compact rendered effectively the same | Full-screen cards now use tap-for-detail, a compact move handle, and overflow rare actions; mobile Comfortable uses a 3-column title grid while Compact uses a denser poster-only grid |
| 2026-07-06 | Treat touch iPads as their own Rank breakpoint | Real Safari screenshots showed desktop-width iPad viewports mixing the two-column shell with touch row controls, collapsing titles and overexposing row actions | Coarse-pointer landscape keeps the two-column shell with compact icon row actions; coarse-pointer portrait uses a ranking-first single-column shell with Add/Move in the ranking header |
| 2026-07-10 | Separate responsive composition from input modality, superseding pointer-driven iPad layout forks | Desktop and iPad should share geometry whenever their available space is comparable, while iPad and iPhone should share touch/pen interaction semantics | Width/height and short-landscape queries own layout; `any-pointer:coarse` owns 44 px targets, overflow actions, Move-mode handles, 16 px editable fields, and hover suppression; modern phone landscape retains the mobile shell despite an 844 px CSS width |
| 2026-07-10 | Use editorial keylines for routine high-value actions and reserve solid black for state | Large black CTA fills made ordinary actions feel harsher than the surrounding monochrome card language, while a universal single outline would flatten the primary/secondary hierarchy | Primary actions and Add/Search use a white double keyline plus heavier label; secondary actions keep one outline; tertiary actions stay quiet; solid black remains for selected controls, progress, content surfaces, and irreversible confirmation |
| 2026-07-10 | Separate pack dismissal from pack-to-pack navigation while matching All Packs | A back arrow used for returning to All Packs was visually grouped with previous/next arrows and could not communicate whether it dismissed the sheet or changed packs; the first `× Close` treatment then introduced a second dismissal pattern | Pack detail now reuses the All Packs icon-only `×` in the upper-right and groups optional Prev / Pack x of y / Next navigation compactly at the toolbar's left edge |
...
| 2026-07-01 | Production release review | Fixed post-release home-screen and action-system defects from supplied screenshots/user reports: content-sized desktop Add/Search rail, content-sized short ranking panel, non-wrapping mobile detail footer actions, mobile detail terminal metadata separator, inconsistent movie action rows, mixed labeled/icon-only mobile ranking toolbar actions, uneven desktop ranking action hover states, stale movie detail content during sequential opens, underpowered comparison-card hero treatment, and the unclear ranking-review Swap glyph; updated cache keys and production smoke expectations | Supplied screenshot classes reproduced and fixed; local screenshots inspected; focused detail, app-shell, comparison, and ranking-review E2E assert the mobile metadata border/action-row/toolbar-label, desktop hover, stale-detail, poster-dominance, title/year proximity, comparison geometry, and text-only Swap regressions; `npm run verify` passed; latest product production deployment `dpl_3N3uKvApJEyi8FPQTxTH8k9M28ov` reached READY; `npm run test:production` passed; final production screenshots at `debug/screenshots/runs/2026-07-01T0738Z-detail-loading-production` and `debug/screenshots/runs/2026-07-01T0816Z-comparison-hero-production`; production console and Vercel runtime logs were inspected | Monitor production for real-user visual reports; no unrelated product work started |
| 2026-07-02 | Mobile density follow-up | Reduced mobile Rank and Lists control chrome after real-device feedback: sticky compact ranking header, row-tap detail, Move mode handles, ranking overflow, queue overflow with only Rank top-level, and refreshed cache keys | In-app browser desktop/mobile review; focused first-run, app-shell, detail, queue, and mobile-pack E2E; `npm run verify` passed (197 Node, 3 Deno, 23 E2E) | Monitor deployed mobile density and gather next feedback on detail, full-screen ranking, and share surfaces |
| 2026-07-02 | Mobile density regression fix | Fixed the clipped/blank mobile ranking overflow menu, carried the compact action hierarchy into full-screen ranking cards, and made Comfortable/Compact density meaningfully distinct | Browser plugin page-health/screenshot check; focused app-shell/full-screen/first-run/privacy E2E; `npm run verify` passed (197 Node, 3 Deno, 24 E2E) | Run production smoke after deployment and keep watching real-device screenshots |
| 2026-07-02 | Overflow dismissal regression fix | Tightened the shared overflow menu interaction contract so outside taps dismiss menus without falling through to row/card primary actions | Browser plugin mobile smoke for main and full-screen ranking; focused app-shell/full-screen E2E now cover outside-tap dismissal; full `npm run verify` passed | Keep watching menu behavior across queues and pack/list rows as more rare actions move into overflow |
| 2026-07-02 | Overflow detail layering fix | Ensured movie detail opened from overflow Info clears overflow state and paints above row/card/menu layers | Browser plugin mobile smoke for main and full-screen ranking; focused app-shell/full-screen E2E now cover overflow Info layering across ranking, queues, and full-screen cards; full `npm run verify` passed | Run production smoke after deployment |
| 2026-07-02 | Overflow Info dismissal hardening | Closed overflow action selection earlier in the event path and added stale-state cleanup/backstop hiding so menus cannot remain above detail sheets | Browser plugin mobile smoke for the exact ranking overflow Info path; app-shell E2E coordinate-taps Info and asserts zero open/visible menus plus detail hit-testing; full `npm run verify` passed; production smoke passed 21 checks against `app.js?v=159` / `styles.css?v=124` | Monitor real-device overflow/menu reports |
| 2026-07-06 | iPad responsive follow-up | Fixed real-device iPad Rank regressions: compact touch-tablet row actions, ranking-first portrait tablet shell, Add/Move header access, pack title wrapping, and `styles.css?v=126` | Focused app-shell E2E now covers 1024×768 landscape and 980×1180 portrait touch tablet cases; inspected generated screenshots; `npm test`, syntax checks, `git diff --check`, focused app-shell E2E, focused Share Studio rerun after one timing flake, and full `npm run test:e2e` passed | Monitor deployed iPad Safari and run production smoke after deployment |
| 2026-07-07 | iPad suggestion follow-up | Fixed real-device iPad suggestion-card regressions by collapsing the narrow Rank rail suggestion grid to one column, keeping full Discover/portrait tablet grids readable, and forcing Rank/Save/Hide into one segmented touch row; bumped `styles.css?v=127` | Focused suggestion E2E now asserts 1024×768 landscape Rank/Discover and 980×1180 portrait Rank/Discover card widths, grid flow, action segments, no overlap, and no horizontal overflow; app-shell E2E passed; final full `npm run verify` passed after rerunning one unrelated fullscreen timing flake | Monitor deployed iPad Safari and run production smoke after deployment |
| 2026-07-07 | iPad Discover pack shelf | Reworked the touch-tablet Discover pack shelf from three full-width rows into one compact three-card row, preserving the three prioritized packs while avoiding a larger six-pack recommendation change; bumped `styles.css?v=128` | App-shell E2E now asserts 1024×768 landscape and 980×1180 portrait touch-tablet pack shelves use three same-row cards with visible actions, bounded collage/title/progress geometry, no overflow, and phone Discover stays one-column; Browser smoke verified local load, Discover nav, `styles.css?v=128`, and console health; full `npm run verify` passed | Monitor deployed iPad Safari and run production smoke after deployment |
| 2026-07-07 | iPad pack CTA alignment | Made Discover shelf and All Packs pack CTAs use the same compact pill sizing, bottom-aligned All Packs progress/CTA rows across varied card copy, preserved phone row behavior, and bumped `styles.css?v=129` | App-shell E2E now asserts compact same-size CTA geometry on iPad landscape/portrait Discover shelves and All Packs overlays, including row-level action/progress alignment; inspected generated iPad screenshots; Browser smoke verified local load and `styles.css?v=129`; full `npm run verify` passed after rerunning the unrelated fullscreen timing check | Monitor deployed iPad Safari and run production smoke after deployment |
| 2026-07-07 | iPad Lists layout | Reworked Lists on tablet widths to use the full page width, a two-column active queue grid, compact Rank plus overflow row controls, and page-level scrolling instead of nested queue scrollports; bumped `styles.css?v=130` | App-shell E2E now seeds multiple saved/hidden movies and asserts iPad landscape/portrait Lists have two columns, no queue scrollport, compact row controls, no horizontal overflow, and phone Lists still keeps its compact behavior; Browser smoke verified local Lists load, `styles.css?v=130`, no console warnings/errors, and visible overflow removal; full `npm run verify` passed | Monitor deployed iPad Safari and run production smoke after deployment |
| 2026-07-08 | Desktop Lists parity | Extended the tablet Lists treatment to all large-screen widths so desktop Watch next/Hidden uses the same two-column active queue grid, compact Rank plus overflow row controls, and page-level scrolling instead of nested queue scrollports; bumped `styles.css?v=137` | App-shell E2E now asserts Desktop plus iPad landscape/portrait Lists have two columns, no queue scrollport, compact row controls, no horizontal overflow, and phone Lists keeps its compact behavior; inspected the generated desktop Lists screenshot; Browser smoke verified local Lists load, `styles.css?v=137`, no console warnings/errors, and visible large-screen grid state; full `npm run verify` passed | Monitor deployed desktop/iPad Lists and run production smoke after deployment |
| 2026-07-08 | Desktop Discover pack parity | Extended the compact iPad Discover pack shelf to desktop large screens so Suggested Movie Packs render as one three-card row instead of three oversized full-width rows; bumped `styles.css?v=138` | App-shell E2E now asserts Desktop plus iPad landscape/portrait Discover shelves use three same-row cards with bounded cover/title/progress/action geometry while phone Discover remains one-column; Browser smoke verified local Discover load, `styles.css?v=138`, no console warnings/errors, and visible compact pack shelf; full `npm run verify` passed | Monitor deployed desktop/iPad Discover and run production smoke after deployment |
| 2026-07-08 | Desktop Lists overflow anchoring | Fixed desktop Watch next/Hidden overflow menus detaching from the clicked kebab in the two-column Lists grid by positioning menus immediately on open and disabling row/card transforms while a shared movie overflow menu is open; bumped `app.js?v=176` and `styles.css?v=139` | App-shell E2E now opens Watch next and Hidden desktop queue overflow menus and asserts the fixed-position menu stays directly below the clicked toggle and within the source card column; Browser smoke reproduced the local desktop Lists overflow path; full `npm run verify` passed | Monitor deployed desktop Lists overflow menus, especially long queue scroll positions |
| 2026-07-09 | Desktop Lists overflow first-frame positioning | Prevented the shared overflow menu from flashing at stale or default coordinates before its fixed-position geometry is applied by hiding open-but-unpositioned menus until `positionMovieOverflowMenu` sets coordinates and marks them positioned; bumped `app.js?v=177` and `styles.css?v=140` | App-shell E2E now asserts open overflow menus are hidden until positioned and visible only after the positioned state is applied, then opens Watch next and Hidden desktop queue menus and verifies anchored geometry; full `npm run verify` passed | Monitor deployed desktop Lists overflow menus for first-frame flashes on slow devices or heavy tabs |
| 2026-07-10 | Cross-device layout and modality audit | Unified desktop/iPad composition around available width while sharing touch interaction rules across iPad/iPhone; fixed phone landscape shell/comparison geometry, safe areas, touch targets, hover gating, ranking and full-screen drag transactions, focus restoration, import cancellation, overlay sizing/scroll isolation, lightbox keyboard/swipe behavior, public-list modal semantics, and narrow tablet/phone card layouts; bumped `app.js?v=178`, `styles.css?v=141`, and `shared.js?v=5` | Deterministic desktop/iPad/iPhone screenshots inspected, in-app Browser exercised desktop/mobile navigation with no horizontal overflow or console warnings/errors, and full `npm run verify` passed (229 Node, 21 Deno, 27 E2E); reports at `reports/runs/2026-07-10T142754Z` and `reports/e2e/runs/2026-07-10T142756Z` | Run production smoke after deployment and monitor physical iPad/iPhone Safari for keyboard, pinch-zoom, and safe-area edge cases |
| 2026-07-10 | Pack dismissal and CTA hierarchy follow-up | Replaced the ambiguous pack back arrow with the same upper-right icon-only `×` used by All Packs, fit the filtered pack pager to its left, and replaced harsh black routine CTA fills with the editorial double-keyline primary hierarchy across Add/Search, packs, Rank actions, detail, Share, sign-in, import, Tonight, full-screen jump, and public shared lists; bumped `app.js?v=179`, `styles.css?v=146` | In-app Browser verified pager-present and pager-absent pack detail states; deterministic desktop/iPad/iPhone screenshots covered All Packs and pack detail; focused close-style parity, alignment, title-clearance, and primary-action regressions added; full `npm run verify` passed (229 Node, 21 Deno, 28 E2E) | Run production smoke after deployment and monitor physical devices for perceived primary/secondary contrast |
| 2026-07-10 | Share lightbox focus fallback | Preserved keyboard focus restoration when an async Share Studio preview rerender replaces the exact element that opened the lightbox; the fallback resolves the current equivalent single preview or image-set card; bumped `app.js?v=180` | E2E now deliberately replaces the single-image preview while its lightbox is open and verifies Escape returns focus to the replacement; focused Share Studio smoke and full `npm run verify` passed | Keep the stable semantic fallback when changing Share preview rendering or lightbox navigation |
| 2026-07-10 | Pack pager grouping refinement | Converted the filtered-pack Prev / count / Next controls from a toolbar-spanning distribution into one intrinsic-width, padded group at the left edge while preserving the independent upper-right close control; bumped `styles.css?v=147` | In-app Browser verified Next interaction and measured 305 px desktop/iPad and 249 px phone pager groups with no overflow; deterministic desktop/iPad/iPhone screenshots inspected; focused pack regressions and full `npm run verify` passed | Monitor physical iPad Safari for the compact pager's perceived spacing at different Dynamic Type settings |
| 2026-07-10 | Action-first home IA | Superseded Rank / Discover / Lists with **Rank / Ranking / Lists**. Rank is now a full-width ranking launchpad combining search, packs, and individual movie prompts; Ranking owns the ordered-list artifact, Recently ranked, and Taste Explorer; Lists remains the queue destination. This is an evolution of the released component system, not a visual revert. | Desktop/iPad/iPhone shell contracts updated in E2E; placement feedback links back to Ranking without pulling the user out of the ranking loop; in-app Browser exercised Rank → comparison → Rank → Ranking with clean console health; deterministic finals at `debug/screenshots/runs/2026-07-10T21-04-01Z-action-first-final`; full `npm run verify` passed (231 Node, 21 Deno, 28 E2E); `app.js?v=181`, `styles.css?v=149`, `lib/app-shell.js?v=2` | Compare ranking-start rate and rankings/session after release; revisit density only with observed funnel evidence |
| 2026-07-10 | First-class fresh pack funnel | Expanded the populated launchpad shelf from three deterministic packs to six rotating packs. Selection preserves at most two explicit continuations and one organic head start, then favors unfinished category diversity; it rerolls per visit and from a visible refresh control. The first-run three-pack curriculum remains deliberate. | Pure seeded selection tests cover completion exclusion, continuation caps, category variety, and rotation; cross-device E2E covers 6-up desktop, 3×2 iPad, and horizontal iPhone shelves | Monitor pack opens, pack-to-ranking starts, refresh usage, and whether six choices creates scanning fatigue |
