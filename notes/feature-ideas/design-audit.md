...
| Ranking comparison UI lacked hero-level visual weight | Supplied desktop screenshot showed generic white cards with undersized posters, separated title/year text, and roughly half the cards reading as empty space | Rebuilt the active comparison cards as poster-led hero choices: larger poster art, blurred artwork color fields, grouped title/date captions, expanded desktop comparison width, and responsive portrait/landscape sizing | In-app Browser real search flow opened American Beauty vs Vertigo and archived desktop/mobile/mobile-landscape screenshots at `debug/screenshots/runs/2026-07-01T0748Z-comparison-hero`; focused comparison E2E now uses poster-backed fixtures and asserts poster dominance, title/year proximity, no overlap, and viewport fit; cache bumped to `app.js?v=148`, `styles.css?v=112` |
