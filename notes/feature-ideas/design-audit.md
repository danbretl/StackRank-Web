...
| 2026-07-01 | Production release review | Fixed post-release home-screen and action-system defects from supplied screenshots: content-sized desktop Add/Search rail, content-sized short ranking panel, non-wrapping mobile detail footer actions, mobile detail terminal metadata separator, inconsistent movie action rows, and mixed labeled/icon-only mobile ranking toolbar actions; updated cache keys and production smoke expectations | Supplied screenshot classes reproduced and fixed; local screenshots inspected; focused detail and app-shell E2E assert the mobile metadata border/action-row/toolbar-label regressions; `npm run verify` passed after the separator fix; production deployment `dpl_3Ek33jwjLN7caL3BxFwNZziQfyvD` reached READY; `npm run test:production` passed; final production screenshots and runtime logs were inspected | Monitor production for real-user visual reports; no unrelated product work started |
