...
- **Public snapshot links** — signed-in users can publish/update/copy/revoke a short `/s/:slug` link from Share Studio. It stores a static minimal snapshot in `shared_lists` and renders a read-only monochrome poster grid with tappable movie-detail sheets and a "Rank your own movies" CTA; signed-out users are prompted to sign in before publishing.

`SHARE_OPTIONS_VERSION` is now 7 (v6 migrated `portrait`/`landscape` → `skinny`/`wide`; v7 added the `packs` toggle). Current cache keys: `app.js?v=173`, `styles.css?v=134`, `shared.js?v=3`.
