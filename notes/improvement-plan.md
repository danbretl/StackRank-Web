...
- **10 Public shareable list link:** implemented in this pass; signed-in users can publish/update/copy/revoke read-only snapshot links from Share Studio, `/s/:slug` renders a static poster-grid page from `shared_lists`, revoked rows disappear from public reads, publish/view telemetry is privacy-bounded, and helper/unit plus mocked Supabase E2E coverage is in place.
- **11 "Pick something for tonight":** implemented 2026-07-09, extended beyond the original sketch with an optional free-text mood/vibe input. A new `tonight-pick` edge function (origin/apikey-gated, rate-limited) enriches queue candidates from TMDB (runtime/genres/keywords/credits, cached) and scores the vibe server-side via a curated lexicon in `_shared/mood.ts` (Deno-tested); `lib/tonight.js` (unit-tested) blends mood fit, rank-weighted taste affinity, soft runtime-window fit, and queue-age freshness into three explained picks with a diversity guard, reshuffle, a "Watch this" choice state, and a Rank-it handoff into the normal comparison flow (`ranking_started` source `tonight`). Telemetry adds `tonight_opened`/`tonight_picked` (migration + `lib/telemetry.js`), never recording the mood text. Full design record: `notes/feature-ideas/pick-something-for-tonight.md`; E2E flow "tonight picker mood, choice, and rank round trip".
