...
H finishes with 48 built-in calls, 47 native outputs, 43 accepted masters, four quality rejections and one failed input call. Five retries comprise four targeted quality corrections plus one documented Hällefors reference-input compatibility retry. All 43 pairs are published; no active pair remains unfinished. Start future work with current 450 coverage and a fresh cohort (I if unused), verifying counts first.
