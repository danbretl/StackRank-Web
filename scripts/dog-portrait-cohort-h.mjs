...
export const hDigest = value => createHash('sha256').update(JSON.stringify(value)).digest('hex');
...
const BASELINE_COMMIT = 'a4ba9d0e4849f43db5f5a30a5f2ead632ddae6dc';

// H owns its 43-pair freeze. E, F and G retain their separate immutable checks.
export function portraitCohortHSelectionDigest(cohort) {
  return hDigest({ cohortId: cohort.cohortId, frozenAt: cohort.frozenAt,
...
export function activeCohortHIds(cohort) {
...
export function summarizePortraitCohortH(cohort) {
  const all = Object.values(cohort.entries);
  const active = activeCohortHIds(cohort).map(id => cohort.entries[id]).filter(Boolean);
...
export function validatePortraitCohortH(cohort, { catalog, rightsLedger, generatedArtwork, profiles } = {}) {
  const errors = [];
  const fail = (ok, message) => { if (!ok) errors.push(message); };
  fail(cohort.schemaVersion === 1 && cohort.cohortId === 'dogs-portraits-h', 'Unknown H cohort schema or identity');
  fail(cohort.targetCompletedCount === 43 && cohort.primary?.length === 43 && cohort.reserves?.length === 10, 'H requires 43 frozen primaries and 10 ordered reserves');
  if (!Array.isArray(cohort.primary) || !Array.isArray(cohort.reserves) || !Array.isArray(cohort.amendments) || !cohort.entries) return [...errors, 'Missing H selection pools, amendments or entries'];
  fail(iso(cohort.frozenAt), 'H freeze needs an explicit timestamp');
  fail(cohort.selectionSha256 === portraitCohortHSelectionDigest(cohort), 'Frozen H selection digest mismatch');
  fail(cohort.baseline?.illustratedCount === 407 && cohort.baseline?.developedCount === 407 && cohort.baseline?.targetTotalCompletedPairs === 450 && cohort.baseline?.selectableCount === 1239 && cohort.baseline?.commit === BASELINE_COMMIT, 'H baseline/target changed');
  fail(cohort.baseline?.generatedCatalogIds?.length === 407 && new Set(cohort.baseline.generatedCatalogIds).size === 407 && cohort.baseline?.heldCatalogIds?.length === 26 && new Set(cohort.baseline.heldCatalogIds).size === 26, 'H must preserve exact published coverage and prior holds');
  for (const key of ['dog-catalog', 'generated-artwork', 'breed-profiles', 'image-rights', 'packs', 'artwork-license-policy', 'generated-artwork-policy']) {
    fail(text(cohort.sourceSnapshots?.[key]?.path) && text(cohort.sourceSnapshots?.[key]?.snapshotPath) && hash(cohort.sourceSnapshots?.[key]?.sha256), `${key}: H baseline source hash missing`);
  }
  ...
  fail(cohort.assignments?.filter(a => ['f_worker_a', 'f_worker_b', 'f_worker_c'].includes(a.worker)).length === 3 && cohort.assignments.filter(a => ['f_worker_a', 'f_worker_b', 'f_worker_c'].includes(a.worker)).every(a => a.model === 'gpt-6-sol' && a.reasoningEffort === 'high') && cohort.assignments.some(a => a.worker === 'primary orchestrator' && a.model === 'not disclosed in runtime'), 'Actual H model assignments changed or invented');
  fail(cohort.imageGeneration?.tool === 'OpenAI built-in imagegen' && cohort.imageGeneration?.underlyingImageModel === 'undisclosed' && cohort.imageGeneration?.promptTemplateVersion === 'dogs-field-guide-v5-cohort-h' && JSON.stringify(cohort.imageGeneration?.nativeDimensions) === '[1536,1024]', 'H generation provenance changed');
  const pools = [...cohort.primary, ...cohort.reserves];
  fail(new Set(pools.map(entry => entry.catalogId)).size === 53, 'Duplicate H selection identity');
  const byId = new Map((catalog?.entities || []).map(entry => [entry.id, entry]));
  cohort.primary.forEach((entry, index) => {
    const id = entry.catalogId;
    fail(entry.ordinal === index + 1 && entry.subwave === (index < 25 ? 'h01' : 'h02') && text(entry.basis) && text(entry.identityReview), `${id}: frozen H order or basis changed`);
  ...
  for (const entry of pools) {
    fail(!cohort.baseline.generatedCatalogIds?.includes(entry.catalogId), `${entry.catalogId}: identity already published at H freeze`);
  ...
  for (const [index, amendment] of cohort.amendments.entries()) {
    ...
    fail(amendment.sequence === index + 1 && amendment.previousSha256 === previousSha256 && sha256 === hDigest(content), 'Append-only H amendment chain mismatch');
    fail(slot >= 0 && cohort.entries[amendment.blockedCatalogId]?.hold?.status === 'blocked', 'H reserve activation requires active blocked identity');
    ...
    fail(next?.catalogId === amendment.activatedCatalogId, 'H must activate next eligible ordered reserve');
    fail(iso(amendment.recordedAt) && text(amendment.reason) && amendment.evidencePaths?.length > 0, 'H activation needs dated evidence');
  ...
  fail(Object.keys(cohort.entries).length === selected.size, 'H progress must retain every primary and activated reserve');
  ...
  for (const id of selected) {
    ...
    if (!entry) { errors.push(`${id}: missing H progress entry`); continue; }
    ...
    for (const [index, attempt] of attempts.entries()) {
      ...
      if (attempt.status === 'generated') fail(iso(attempt.completedAt) && Date.parse(attempt.completedAt) >= Date.parse(attempt.startedAt) && attempt.masterPath?.startsWith('assets/dogs/generated-masters/cohort-h/') && hash(attempt.masterSha256) && attempt.masterSha256 === attempt.originalOutputSha256 && text(attempt.originalOutputPath) && attempt.width === 1536 && attempt.height === 1024, `${id}: native generated master/hash receipt invalid`);
    ...
    if (entry.integration.status === 'integrated') {
      const wave = active.indexOf(id) < 25 ? 'h01' : 'h02';
      fail(active.includes(id) && entry.qa.status === 'approved' && entry.integration.subwave === wave && entry.integration.batchManifest === `data/dogs/generated-artwork-batch-${wave}.json` && iso(entry.integration.integratedAt) && text(entry.qa.contactSheetPath), `${id}: only accepted H pairs may integrate into their frozen wave`);
      if (generatedArtwork) {
      ...
      if (profiles) fail(profiles.profiles?.[id]?.reviewStatus === 'editor-reviewed' && hDigest(profiles.profiles[id].summary) === entry.profile.summarySha256, `${id}: integrated reviewed summary mismatch`);
    }
  ...
  fail(JSON.stringify(cohort.progress) === JSON.stringify(summarizePortraitCohortH(cohort)), 'H progress summary is stale');
  return errors;
}
